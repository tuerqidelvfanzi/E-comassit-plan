#!/usr/bin/env python3
# build_deep_v201.py — 架构图深层重新分析 V2.0.1 生成器
#
# V2.0.1 (Y 小版本: 新增"用户故事地图"分页, 12→13 张)
#   - 补全 V2.0.0 缺失的"用户故事"前置叙事页
#   - 不是"端到端"步骤 (用户说太公式化), 用 Jeff Patton User Story Mapping 方法
#   - 1 角色 + 3 活动 + 5 故事 + 1 成功标准 = 8 节点
#   - 编号 C0 (与 V2.0.0 的 C1-C12 区分)
#
# V2.0.0 (X 中版本: 以 C1=20 节点为基准, 全图重做)
#   - 264→235 节点 (-11%), 12 张图
# V1.0.9: 264 节点, 12 张图
import re
...
# 当前版本: V2.0.1 (绝不覆盖历史版本)
DST = ROOT / "docs/architecture-diagrams/架构图深层重新分析V2.0.1.drawio"
# 历史基线 (只读)
V109 = ROOT / "docs/architecture-diagrams/架构图深层重新分析V1.0.9.drawio"
V200 = ROOT / "docs/architecture-diagrams/架构图深层重新分析V2.0.0.drawio"

# ====== 节点预算 (V2.0.1: 新增 C0 用户故事地图, 12→13 张) ======
NODE_BUDGETS = {
    "C0":   8,  # V2.0.1 新增: 用户故事地图 (1 角色 + 3 活动 + 5 故事 + 1 成功标准)
    "C1":  20,  # V1.0.9 原版 (用户觉得清晰, 完美对齐)
    "C2":  17,  # V1.0.9 原版 (17 节点, 9 步 + 4 泳道 + 用户故事 + 图例 + 契约)
    "C3":  18,  # 适度简化: V1.0.9=30, C4 Landscape 业内 ≤10, 折中 3 源×3 目标 (18 节点)
    "C4":  22,  # V1.0.9 原版
    "C5":  19,  # V1.0.9 原版 (4 阶段 × 4 子节点)
    "C6":  22,  # V1.0.9 原版 (4 段 × 3 + 共享 + 状态 + 路径)
    "C7":  18,  # V1.0.9 原版 (4 列 × 2 + 重试 + 校验 + P2)
    "C8":  22,  # 适度简化: V1.0.9=33, 砍 White Hook 等细节, 5 实体 × 2 + 6 段 × 2
    "C9":  25,  # 适度简化: V1.0.9=44, 7 域→5 大类, 每类 4-5 端点
    "C10":  9,  # V1.0.9 原版
    "C11": 22,  # V1.0.9 原版 (3 组件 + 2 部署 + 4 环境 × 2)
    "C12": 24,  # 适度简化: V1.0.9=27, 砍 ERP 边界, 保留 5 ADR + 5 风险 (24 节点, 视觉清晰)
}
TOTAL_BUDGET = sum(NODE_BUDGETS.values())  # 246 (238 + 8)
...
def node_unlimited(x, y, w, h, label, style=STYLE_DEFAULT):
    """不限制行数的节点 (V2.0.1+ 用于 C0 用户故事表, 避免 clip2 截断)

    用法: 当 value 超过 2 行时, 用此函数代替 node()
    """
    cid = nid()
    GEOM[cid] = (x, y, w, h)
    safe = xml_attr(label)  # 不用 clip2
    return (cid, f'<mxCell id="{cid}" value="{safe}" style="{style}" vertex="1" parent="1"><mxGeometry x="{x}" y="{y}" width="{w}" height="{h}" as="geometry"/></mxCell>')
...
def page_c0():
    ...
    设计原则 (Jeff Patton User Story Mapping):
    - 横向: 3 大活动 (选品/改写/上架)
    - 纵向: 1 角色 + 故事 (合并 1 表格) + 1 成功标准
    - 不是"端到端步骤" (用户说太公式化), 是"叙事性用户故事"
    - 节点数: 1 标题 + 1 角色 + 3 活动 + 1 故事表 + 1 成功标准 = 7 节点
    """
    reset_ids()
    ...
    # 1. 标题 (含本应用定位)
    nid_t, s = node(50, 30, 1180, 60,
                    "用户故事地图 (C0 User Story Mapping)  ·  本应用 = 小卖家 AI 助手 (单店+店群, 多对多东南亚)",
                    STYLE_TITLE)
    out.append(s)

    # 2. 角色 (1 节点, persona)
    nid_p, s = node(50, 130, 200, 100,
                    "运营\n(单店为主\n+店群)",
                    STYLE_ACTOR)
    out.append(s)
    ...
    for x, title, sub in STAGES:
        nid_s, s = node(x, 130, 280, 100, f"{title}\n{sub}", STYLE_PHASE)
        out.append(s)
        stage_ids.append(nid_s)

    # 4. 6 个故事 (1 表格节点, 用 node_unlimited 突破 2 行限制)
    # 故事基于代码实际状态 (✅ P0 = 已实现, 🚧 = 部分实现, 📋 P1 = 规划中)
    nid_stories, s = node_unlimited(50, 280, 1180, 200,
                          "6 个核心用户故事 (As a / I want / So that, 基于代码事实):\n"
                          "  ① 作为 运营, 我想看到采集箱列表 (/products), 从而 找到待处理商品  [✅ P0]\n"
                          "  ② 作为 运营, 我想看竞品 TOP10 (/v2/competitor-jobs), 从而 选品有依据  [📋 P1]\n"
                          "  ③ 作为 运营, 我想选 1 个商品, 从而 进入采集流程  [✅ P0]\n"
                          "  ④ 作为 AI 助手, 我想改写标题/图/价 (shared/listing/*), 从而 符合各平台规则  [🚧 P0 越南 / P1 3 站]\n"
                          "  ⑤ 作为 MV3 插件, 我想自动填表上架, 从而 节省 25 分钟/品  [🚧 P0 越南 / P1 3 站]\n"
                          "  ⑥ 作为 运营, 我想看回写状态 (publish_tasks.status), 从而 失败可重试  [✅ P0]",
                          STYLE_DATA)
    out.append(s)

    # 5. 成功标准 + 交叉引用 + 价值三角镜 (合并 1 节点)
    nid_o, s = node_unlimited(50, 480, 1180, 220,
                    "Outcome 成功标准:\n"
                    "  · 单品上架 ≤ 5 分钟 (传统 30-60 分钟, 提速 6-12 倍)\n"
                    "  · 单店日上架 50-80 品  ·  店群多店并行  ·  边界清晰 (不做 ERP)\n"
                    "交叉引用: 选品→C3 多对多矩阵  ·  改写→C6 管线  ·  上架→C7 发布  ·  状态→C10  ·  边界→C12 ADR-5\n"
                    "价值三角镜: A 高频重复→规则 | B 高频模糊→AI 辅助 | C 中频→选品 | D 排除→不做 ERP",
                    STYLE_BUS)
    out.append(s)

    # 边 (角色→3 活动, 活动之间, 故事→成功标准)
    out.append(edge(nid_p, stage_ids[0], "看", hint={'src':'right','tgt':'left'})[1])
    out.append(edge(stage_ids[0], stage_ids[1], "改", hint={'src':'right','tgt':'left'})[1])
    out.append(edge(stage_ids[1], stage_ids[2], "发", hint={'src':'right','tgt':'left'})[1])
    out.append(edge(stage_ids[2], nid_stories, "→", hint={'src':'bot','tgt':'top'})[1])
    out.append(edge(nid_stories, nid_o, "→", hint={'src':'bot','tgt':'top'})[1])

    n_edges = sum(1 for s in out if 'edge="1"' in s)
    n = len(out) - n_edges
    assert n <= NODE_BUDGETS["C0"], f"C0 节点 {n} 超预算 {NODE_BUDGETS['C0']}"
...
def page_c3():
    """C3 多对多平台矩阵 · 3 源 × 3 目标 + 简表 + 行头→单元连接线 (V2.0.1 修复)"""
    reset_ids()
    page_w, page_h = 1400, 800
    out = []

    # 标题 (2 行, 含图例 + 读法)
    nid_t, s = node(50, 20, 1300, 80,
                    "多对多平台矩阵  ·  ✅ P0 已跑通  ·  📋 P1 规划  ·  ❌ OOS\n"
                    "读法: 纵轴=源平台, 横轴=目标平台, 单元格=适配状态 (实际跑通的 P0)",
                    STYLE_TITLE)
    out.append(s)
    ...
    for col, name in enumerate(targets):
        nid_h, s = node(300 + col*330, 130, 300, 50, name, STYLE_PHASE)
        out.append(s)

    # 行头：3 源 (V1.0.9 = 5, 简化为 3 P0/P1 重要的)
    sources = ["1688", "Shopee 多站(源)", "链接直采 (Worker)"]
    row_ids = []
    for row, name in enumerate(sources):
        nid_h, s = node(50, 200 + row*90, 240, 70, name, STYLE_DEFAULT)
        out.append(s)
        row_ids.append(nid_h)

    # 矩阵单元 (3×3=9) - 2 行内容 (clip2 截到, 不会丢字)
    cell_labels = [
        # 越南, 泰国, 菲律宾
        [('✅ P0',  '端到端'), ('📋 P1',  '规划中'), ('✅ P0',  '端到端')],
        [('✅ P0',  '端到端'), ('📋 P1',  '规划中'), ('✅ P0',  '端到端')],
        [('✅ P0',  '通用'),   ('✅ P0',  '通用'),   ('✅ P0',  '通用')],
    ]
    cell_ids = [[None]*3 for _ in range(3)]
    for row in range(3):
        for col in range(3):
            status, sublabel = cell_labels[row][col]
            if '✅' in status:
                color = STYLE_DATA
            else:
                color = STYLE_PHASE
            label = f"{status}\n{sublabel}"
            nid_m, s = node(300 + col*330, 200 + row*90, 300, 70, label, color)
            out.append(s)
            cell_ids[row][col] = nid_m

    # 边: 行头 → 矩阵单元 (9 条, endArrow=none 表示归属线, 不带标签)
    for row in range(3):
        for col in range(3):
            out.append(edge(row_ids[row], cell_ids[row][col],
                            style="endArrow=none;strokeColor=#cccccc;",
                            hint={'src':'right', 'tgt':'left'})[1])

    # 范围说明
    nid_oos, s = node(50, 530, 1300, 80,
                      "范围: v2 聚焦东南亚 Shopee + TikTok, 不做淘宝/天猫作为目标\n"
                      "排除: 拼多多反爬强(平台跨境不支持) · 完整 P1 矩阵见 FIELD_CHECKLIST.md",
                      STYLE_BUS)
    out.append(s)
...
def page_c8():
    """C8 数据模型+6段SKU · 5 实体 + 6 段 SKU (V2.0.0 适度简化, 砍 7→5 实体, 砍 6 段 desc)"""
    reset_ids()
    ...
    # 5 个核心实体 (砍掉 desc 描述, 1 节点/实体)
    entities = [
        (50, 100, "users",        "用户表\n存 bcrypt 哈希"),
        (380, 100, "products",     "商品表 (核心)\nrawCapture 快照"),
        (710, 100, "collect_jobs", "采集任务\nNormalizedProduct"),
        (1040, 100, "pipeline_runs","管线运行\nProcessedOutput"),
        (380, 250, "publish_tasks","发布任务\nlisting 载荷"),
    ]
    ent_ids = []
    ...
    # 6 段 SKU 结构标题
    nid_sk, s = node(50, 430, 1400, 50, "6 段 SKU 结构: PREFIX-SEQ-SIDE-COLOR-SIZE-(+B|+H)", STYLE_TITLE)
    out.append(s)

    # 6 段 (V1.0.9 有 desc 节点, V2.0.0 砍 desc, 只留段名)
    segs = [
        (50, "PREFIX"),
        (290, "SEQ"),
        (530, "SIDE"),
    ...
    seg_ids = []
    for x, name in segs:
        nid_n, s = node(x, 510, 200, 50, name, STYLE_PHASE)
        out.append(s)
    ...
    # 段说明 (1 节点含全部)
    nid_sku_desc, s = node(50, 600, 1400, 60,
                           "PREFIX: 类目前缀 BF=童装  ·  SEQ: 3位序号  ·  SIDE: P=正面/R=反面/PR=双面  ·  COLOR: WH/BK/PK  ·  SIZE: 110cm/120cm  ·  后缀: +B=印花/+H=热转印",
                           STYLE_DEFAULT)
    out.append(s)

    # White Hook 例子
    nid_wh, s = node(50, 700, 1400, 50,
                     "White Hook (诱饵) 例子: BF-9999-P-WH-110   (价 400 / 库存 5 / 重 220g — 防平台 0 销量降权)",
                     STYLE_BUS)
    out.append(s)

    # 状态 + 状态图例 (合并 1 节点)
    nid_legend, s = node(50, 780, 1400, 80,
                         "3 套状态: products 4 态 | publish_tasks 6 态 (后端含 filling) | ListingStatus 5 态 (UI 简化)\n"
                         "为什么 3 套?  后端 6 态完整, UI 5 态, 采集箱 4 态 — 各角色只关心自己要看的",
                         STYLE_PHASE)
    out.append(s)
...
def page_c9():
    """C9 API 总图 · 5 域 (V2.0.0 合并 7→5 域, 每域 ≤3 端点)"""
    reset_ids()
    page_w, page_h = 1500, 1000
    out = []

    nid_t, s = node(500, 20, 500, 50, "API 总图 · 5 大类 · 26 端点", STYLE_TITLE)
    out.append(s)

    # 顶部说明
    nid_top, s = node(50, 80, 1400, 50,
                      "V1.0.9=7 域 26 端点 · V2.0.0 合并为 5 大类 (Auth+Extension 合并) · 包络: {code, message, data}",
                      STYLE_PHASE)
    out.append(s)

    # 5 大类 (合并 Auth+Extension, Insights 简化为 1 域)
    # 每域: 1 标题(含业务作用) + 3 端点 = 4 节点
    domains = [
        (50, "Auth 认证 (合并 Extension)",
         ["POST /auth/login", "GET /auth/me", "POST /extension/token"]),
        (320, "Collect 采集",
         ["POST /collect-jobs", "GET /collect-jobs/:id", "POST /collect-jobs/:id/parse"]),
        (590, "Products 商品",
         ["GET /products", "PATCH /products/:id", "DELETE /products/:id"]),
        (860, "Pipeline + Insights",
         ["POST /products/:id/pipeline-runs", "GET /pipeline-runs/:id", "POST /insights/competitors"]),
        (1130, "Publish 发布",
         ["POST /publish-tasks", "PATCH /publish-tasks/:id", "POST /publish-tasks/:id/validate"]),
    ]

    for x, name, eps in domains:
        # 域头 (含业务作用)
        nid_h, s = node(x, 150, 240, 80, name, STYLE_PHASE)
        out.append(s)
        # 端点列表 (每域 3 个)
        for i, ep in enumerate(eps):
            y = 250 + i * 45
            nid_e, s = node(x, y, 240, 38, ep, STYLE_DEFAULT)
            out.append(s)
            out.append(edge(nid_h, nid_e, hint={'src':'bot', 'tgt':'top'})[1])

    # 底部包络
    nid_env, s = node(50, 460, 1400, 80,
                      "响应包络: { code: 0, message: \"ok\", data: T }   ·  code≠0 = 业务错误\n"
                      "鉴权: JWT (jose) + bcryptjs  ·  校验: Zod",
                      STYLE_PHASE)
    out.append(s)

    # 状态/计划
    nid_p, s = node(50, 570, 1400, 100,
...
def page_c12():
    """C12 风险+ADR 决策 · 5 ADR + 5 风险 (V2.0.0 简化 6→5 风险, ERP 边界并入 ADR-5)"""
    reset_ids()
    ...
    adrs = [
        ...
        ("ADR-5 不做 ERP (含 ERP 边界)",
         "Why: ERP 涉及订单/仓储/财务, 复杂度指数级增长; 本项目专注\"商品上架\"这一段\nStatus: ✅ 硬约束: 不做订单/仓储/财务/多店防关联/学员账号"),
    ]
    ...
    # 5 风险 (V1.0.9 = 6, 合并 R-5/R-6)
    nid_rh, s = node(50, 660, 1400, 40, "5 大风险 (含 风险是什么 / 缓解措施 / 状态)", STYLE_PHASE)
    out.append(s)
    risks = [
        ...
        ("R-5 多店/计费\n(店群+LLM token 成本)",
         "缓解: ADS Power 集成(P2) + /billing/hooks(P2)  ·  v2 不实现\n状态: 📋 P2 规划"),
    ]
...
def main_v201():
    """V2.0.1 主入口: 新增 C0 用户故事地图, 12→13 张"""
    p0w, p0h, p0nodes = page_c0()
    ...
    diag_c0 = build_diagram(0, "C0-用户故事地图", p0w, p0h, p0nodes)
...
'''
    ...
    assert len(ids) == 13, f"应有 13 张图, 实际 {len(ids)} 张"
    assert len(set(ids)) == len(ids), f"图 ID 重复: {[i for i in ids if ids.count(i)>1]}"
    print(f"  [校验] 13 张图 ID 唯一")

    # 节点预算验证 (C4 业内硬约束)
    def count_nodes(p):
        return sum(1 for s in p if 'edge="1"' not in s)
    counts = [
        ("C0",  count_nodes(p0nodes),  NODE_BUDGETS["C0"]),
        ("C1",  count_nodes(p1nodes),  NODE_BUDGETS["C1"]),
        ("C2",  count_nodes(p2nodes),  NODE_BUDGETS["C2"]),
        ("C3",  count_nodes(p3nodes),  NODE_BUDGETS["C3"]),
        ("C4",  count_nodes(p4nodes),  NODE_BUDGETS["C4"]),
        ("C5",  count_nodes(p5nodes),  NODE_BUDGETS["C5"]),
        ("C6",  count_nodes(p6nodes),  NODE_BUDGETS["C6"]),
        ("C7",  count_nodes(p7nodes),  NODE_BUDGETS["C7"]),
        ("C8",  count_nodes(p8nodes),  NODE_BUDGETS["C8"]),
        ("C9",  count_nodes(p9nodes),  NODE_BUDGETS["C9"]),
        ("C10", count_nodes(p10nodes), NODE_BUDGETS["C10"]),
        ("C11", count_nodes(p11nodes), NODE_BUDGETS["C11"]),
        ("C12", count_nodes(p12nodes), NODE_BUDGETS["C12"]),
    ]
    total = 0
    for name, actual, budget in counts:
        marker = "OK" if actual <= budget else "OVER!"
        print(f"  [{name}] {actual:>3} nodes / budget {budget}  [{marker}]")
        total += actual
    print(f"  [TOTAL] {total} nodes (V1.0.9 = 264, V2.0.0 = 235, budget = {TOTAL_BUDGET})")
    assert total <= TOTAL_BUDGET, f"total nodes {total} over budget {TOTAL_BUDGET}"
    ...
    forbidden = {
        ...
        "架构图深层重新分析V2.0.0.drawio",
    }
    if DST.name in forbidden:
        raise RuntimeError(f"拒绝覆盖历史基线 {DST.name}! 改 DST 路径.")
    print(f"[V2.0.1] {DST.name}")
    print(f"  C0:  {len(p0nodes):>3} cells, page {p0w}x{p0h}")
    print(f"  C1:  {len(p1nodes):>3} cells, page {p1w}x{p1h}")
    print(f"  C2:  {len(p2nodes):>3} cells, page {p2w}x{p2h}")
    print(f"  C3:  {len(p3nodes):>3} cells, page {p3w}x{p3h}")
    print(f"  C4:  {len(p4nodes):>3} cells, page {p4w}x{p4h}")
    print(f"  C5:  {len(p5nodes):>3} cells, page {p5w}x{p5h}")
    print(f"  C6:  {len(p6nodes):>3} cells, page {p6w}x{p6h}")
    print(f"  C7:  {len(p7nodes):>3} cells, page {p7w}x{p7h}")
    print(f"  C8:  {len(p8nodes):>3} cells, page {p8w}x{p8h}")
    print(f"  C9:  {len(p9nodes):>3} cells, page {p9w}x{p9h}")
    print(f"  C10: {len(p10nodes):>3} cells, page {p10w}x{p10h}")
    print(f"  C11: {len(p11nodes):>3} cells, page {p11w}x{p11h}")
    print(f"  C12: {len(p12nodes):>3} cells, page {p12w}x{p12h}")
