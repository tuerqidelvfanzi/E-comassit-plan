#!/usr/bin/env python3
# build_deep_v200.py — 架构图深层重新分析 V2.0.0 生成器
#
# 以 V1.0.9 的 C1 业务全景图 (20 节点) 为基准, 全部 12 张图按"清晰明了"风格重做
# 保留 V1.0.9 的视觉风格 + 实现状态标记, 适度应用 C4 Model 业内标准简化
#
# V2.0.0 (X 中版本: 以 C1=20 节点为基准, 全图重做)
#   - 节点目标 (参考 V1.0.9 各图实际密度):
#     C1 业务全景 20  | C2 端到端 14 | C3 多对多 12 (Landscape 简表)
#     C4 系统架构 22  | C5 抓数据 16 | C6 改写 17
#     C7 写数据 16    | C8 数据模型 18 (C4 ER 简化) | C9 API 22 (C4 Component 简化)
#     C10 状态机 9    | C11 部署 16 | C12 风险+ADR 16 (C4 不混层)
#   - 保留 V1.0.9 关键设计: "本应用"称谓, 实现状态标记, 用户故事, ERP边界
#   - 应用 C4 业内硬约束: 不混层, 每条边有 intent 标签
#   - 12 张图 (保持 12 张, 不拆)
import re
from pathlib import Path

ROOT = Path("d:/02-学习/06-yitang/app")
# 当前版本: V2.0.0 (绝不覆盖历史版本)
DST = ROOT / "docs/architecture-diagrams/架构图深层重新分析V2.0.0.drawio"
# 历史基线 (只读)
V109 = ROOT / "docs/architecture-diagrams/架构图深层重新分析V1.0.9.drawio"

# ====== 节点预算 (以 V1.0.9 C1=20 为基准, V1.0.9 实际值, 只对 C3/C8/C9/C12 做适度精简) ======
NODE_BUDGETS = {
...
TOTAL_BUDGET = sum(NODE_BUDGETS.values())  # 238
...
def main_v2():
    """V2.0.0 主入口: 以 V1.0.9 C1=20 为基准, 全图重做"""
...
'''
    ...
    for name, actual, budget in counts:
        ...
        total += actual
    print(f"  [TOTAL] {total} nodes (V1.0.9 = 264, budget = {TOTAL_BUDGET})")
