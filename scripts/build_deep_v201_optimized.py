...
def xml_attr(s: str) -> str:
    """优化版: html.escape (C) + lru_cache (重复 value 缓存)"""
...
def page_c0():
    ...
    # 4. 5 个故事 (1 表格节点, 合并 5 个故事)
    nid_stories, s = node(50, 280, 1180, 180,
                          "5 个核心用户故事 (As a / I want / So that):\n"
                          "  ① 作为 运营, 我想看到竞品 TOP10, 从而 选品有依据  [✅ P0]\n"
                          "  ② 作为 运营, 我想选 1 个商品, 从而 进入采集  [✅ P0]\n"
                          "  ③ 作为 AI 助手, 我想改写标题/图/价, 从而 符合各平台规则  [🚧 P0/P1]\n"
                          "  ④ 作为 插件, 我想自动填表上架, 从而 节省 25 分钟/品  [🚧 1 站]\n"
                          "  ⑤ 作为 运营, 我想看回写状态, 从而 失败可重试  [✅ P0]",
                          STYLE_DATA)
    out.append(s)

    # 5. 成功标准 + 交叉引用 + 价值三角镜 (合并 1 节点)
    nid_o, s = node(50, 500, 1180, 220,
                    "Outcome 成功标准:\n"
...
def page_c3():
    """C3 多对多平台矩阵 · 3 源 × 3 目标 + 简表 (V2.0.0 适度简化)"""
    reset_ids()
    ...
    for row in range(3):
        for col in range(3):
            ...
            if '✅' in status:
                ...
                color = STYLE_EXTERN
            nid_m, s = node(300 + col*330, 200 + row*90, 300, 70, status, color)
            out.append(s)
